...
void SimpleShadowmapRender::InitPresentation(VkSurfaceKHR &a_surface, bool)
{
  ...
  m_pVsmData    = std::make_shared<vk_utils::RenderTarget>(m_device, VkExtent2D{ 2048, 2048 });
  ...
  auto memReqDepth            = m_pShadowMap2->GetMemoryRequirements()[0];

  vk_utils::AttachmentInfo infoVsmData;
  infoVsmData.format           = VK_FORMAT_R32G32_SFLOAT;
  infoVsmData.usage            = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT;
  infoVsmData.imageSampleCount = VK_SAMPLE_COUNT_1_BIT;
  m_vsmDataId                  = m_pVsmData->CreateAttachment(infoVsmData);
  auto memReqVsm               = m_pVsmData->GetMemoryRequirements()[0];
  ...
  // memory for all shadowmaps (well, if you have them more than 1 ...)
  {
    ...
    allocateInfo.allocationSize       = memReqDepth.size;
    allocateInfo.memoryTypeIndex      = vk_utils::findMemoryType(memReqDepth.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_physicalDevice);
...

  {
    ...
    allocateInfo.sType                = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    allocateInfo.pNext                = nullptr;
    allocateInfo.allocationSize       = memReqVsm.size;
    allocateInfo.memoryTypeIndex      = vk_utils::findMemoryType(memReqVsm.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_physicalDevice);

    VK_CHECK_RESULT(vkAllocateMemory(m_device, &allocateInfo, NULL, &m_memVsmData));
  }
  m_pVsmData->CreateViewAndBindMemory(m_memVsmData, { 0 });
  {
    ...
    vkCreateSampler(m_device, &samplerInfo, nullptr, &m_vsmDataSampler);
  }
  m_pVsmData->CreateDefaultRenderPass();
...
void SimpleShadowmapRender::SetupSimplePipeline()
{
  ...
  auto vsmData   = m_pVsmData->m_attachments[m_vsmDataId];

  m_pBindings->BindBegin(VK_SHADER_STAGE_FRAGMENT_BIT);
  m_pBindings->BindBuffer(0, m_ubo, VK_NULL_HANDLE, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER);
  m_pBindings->BindImage(1, vsmData.view, m_vsmDataSampler, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
  ...
  m_pBindings->BindEnd(&m_vsmDataDSet, &m_vsmDataDSetLayout);
  ...
  m_pBindings->BindBegin(VK_SHADER_STAGE_FRAGMENT_BIT);
  m_pBindings->BindImage(0, vsmData.view, m_vsmDataSampler, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
  ...
  // pipeline for making shadowmap data useful for VSM
  shader_paths.clear();
  shader_paths[VK_SHADER_STAGE_VERTEX_BIT]   = "../resources/shaders/quad3_vert.vert.spv";
  shader_paths[VK_SHADER_STAGE_FRAGMENT_BIT] = "../resources/shaders/sm2vsm.frag.spv";
  maker.LoadShaders(m_device, shader_paths);
  maker.viewport.width  = float(m_pShadowMap2->m_resolution.width);
  maker.viewport.height = float(m_pShadowMap2->m_resolution.height);
  maker.scissor.extent  = VkExtent2D{ uint32_t(m_pShadowMap2->m_resolution.width), uint32_t(m_pShadowMap2->m_resolution.height) };
  ...
  m_vsmDataPipeline.layout   = maker.MakeLayout(m_device, { m_vsmDataDSetLayout }, sizeof(pushConst2M));
  m_vsmDataPipeline.pipeline = maker.MakePipeline(m_device, emptyInputState, m_pVsmData->m_renderPass);
}
...
void SimpleShadowmapRender::UpdateUniformBuffer(float a_time)
{
  ...
  pushConst2M.vsmEnabled = m_vsmEnabled;
}
                                                     ...
                                                     VkImageView a_targetImageView, VkPipeline a_pipeline)
{
  ...
  //// draw vsm from shadowmap
  //
  VkClearValue clearColor             = {};
  clear                               = { clearColor };
  VkRenderPassBeginInfo renderVsmData = m_pVsmData->GetRenderPassBeginInfo(0, clear);
  vkCmdBeginRenderPass(a_cmdBuff, &renderVsmData, VK_SUBPASS_CONTENTS_INLINE);
  {
    vkCmdBindPipeline(a_cmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, m_vsmDataPipeline.pipeline);
    vkCmdBindDescriptorSets(a_cmdBuff, VK_PIPELINE_BIND_POINT_GRAPHICS, m_vsmDataPipeline.layout, 0, 1, &m_vsmDataDSet, 0, NULL);
    vkCmdPushConstants(a_cmdBuff, m_vsmDataPipeline.layout, VK_SHADER_STAGE_FRAGMENT_BIT | VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(pushConst2M), &pushConst2M);
...
void SimpleShadowmapRender::SetupGUIElements()
{
  ...
  ImGui::NewFrame();
  {
    ...
    ImGui::Checkbox("Enable VSM", &m_vsmEnabled);
